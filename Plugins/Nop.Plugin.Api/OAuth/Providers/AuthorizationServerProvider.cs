...
namespace Nop.Plugin.Api.Owin.OAuth.Providers
{
    public class AuthorisationServerProvider : OAuthAuthorizationServerProvider
    {
        ...
        public override async Task GrantClientCredentials(OAuthGrantClientCredentialsContext context)
        {
            ...
            System.Guid outclientid;
            System.Guid.TryParse(context.ClientId, out outclientid);
            //validate aginstdb or config: GetByClientId(clientId);
            /*bool clientExists = Client client = clientService.GetClient(clientId);
            if (!client)
            {
            ...
            //claimsIdentity.AddClaim(new Claim("LoggedOn", System.DateTime.Now.ToString()));
            claimsIdentity.AddClaim(new Claim("client_id", context.ClientId));

...
namespace Nop.Plugin.Api.Services
{
    public class CustomerApiService : ICustomerApiService
    {
        ...
        public int GetIdByUserId(string userid, bool showDeleted = false)
        {
            if (userid == "")
                return 0;

            int id = 0;
            Customer currentCustomer = _customerRepository.TableNoTracking.FirstOrDefault(customer => customer.UserID == userid);
        ...
        public int GetIdByEmail(string email, bool showDeleted = false)
        {
            if (email == "")
                return 0;

            int id = 0;
            Customer currentCustomer = _customerRepository.TableNoTracking.FirstOrDefault(customer => customer.Email == email);
